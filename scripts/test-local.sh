...
kubectl get pods -n cert-manager -l app=cert-manager | grep -q Running || fail "cert-manager not running"
pass "cert-manager running"
...
# Test via localhost (mapped via --exposed-ports and hostNetwork)
# With TLS configured, nginx redirects HTTP->HTTPS (308). Use HTTPS with -k to skip cert verification.
ENDPOINT="https://localhost"

# Test app 1 (test.justinmcintyre.com)
RESPONSE=$(curl -sk -o /dev/null -w "%{http_code}" -H "Host: test.justinmcintyre.com" "$ENDPOINT") || fail "curl failed"
[[ "$RESPONSE" == "200" ]] || fail "test.justinmcintyre.com: expected 200, got $RESPONSE"
pass "test.justinmcintyre.com returns 200"

curl -sk -H "Host: test.justinmcintyre.com" "$ENDPOINT" | grep -q "It works!" || fail "test.justinmcintyre.com: wrong content"
pass "test.justinmcintyre.com content verified"

# Test app 2 (test2.justinmcintyre.com)
RESPONSE=$(curl -sk -o /dev/null -w "%{http_code}" -H "Host: test2.justinmcintyre.com" "$ENDPOINT") || fail "curl failed"
...
echo ""
echo "Note: Local tests use self-signed certificates (nginx default)."
echo "      Let's Encrypt certificates will be issued on GCP deployment."
